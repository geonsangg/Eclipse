...
public class RateCalcServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command != null && command.equals("calculate")) {
			...
			out.print("<a href='/servletBase02/calc'>환율계산기</a>");
			out.print("</body>");
			out.print("</html>");
		}else {
			out.print("<html><title>환율계산기</title>");
			out.print("<body><h2>환율계산기</h2>");
			out.print("<form name='frmCalc' action='/servletBase02/calc' method='get'>");
	...
	private static String calculate(float won, String operator) {
		...
		if(operator.equals("dollar")) {
			result = String.format("%.6f", won/USD_RATE);  //%.6 = 앞에는 넣은만큼.6 (소수점은 6자리까지) 즉, won/USE_RATE값을 소수6자리까지 한다.
